...
class Docblockr {
  ...
  static activate () {
    ...
    Docblockr.deactivate = (new CompositeDisposable(
      ...
      atom.commands.add('atom-text-editor', 'docblockr:parse-enter', event => {
        ...
        } else if ((docblockr.editorSettings.extend_double_slash === true) && (docblockr.validateRequest({ preceding: /^\s*(\/\/[/!]?|#)/, scope: 'comment.line' }))) {
  ...
  static consumeSnippetsService (service) {
    docblockr.setSnippetsService(service);
    return { dispose: () => docblockr.setSnippetsService(null) };
  }

  constructor () {
    const settings = atom.config.get('docblockr');
  ...
  decorateMultilineCommand (event) {
    ...
    const pos = editor.getCursorBufferPosition();
    const whitespaceRe = /^(\s*)\/\*/;
    const tabSize = atom.config.get('editor.tabLength');
  ...
  wrapLinesCommand (event) {
    ...
    const pos = editor.getCursorBufferPosition();
    // const tabSize = atom.config.get('editor.tabLength');
    const wrapLen = atom.config.get('editor.preferredLineLength');
