...
class Docblockr {
    ...
    Docblockr.deactivate = (new CompositeDisposable(
      ...
      atom.commands.add('atom-text-editor', 'docblockr:parse-enter', event => {
        if (docblockr.validateRequest({ preceding: /^\s*(\/\*([*!])|###\*|\/\/\*)\s*$/ })) {
          ...
          docblockr.parseBlockCommand(event);
          ...
          editor.insertText('\n' + lineText);
        } else if (docblockr.validateRequest({ preceding: /^\s*\*(?:.?|.*(?:[^*][^/]|[^*]\/|\*[^/]))\s*$/, scope: 'comment.block' })) {
  ...
  parseCommand (event, inline) {
    ...
    const out = this.parser.parse(this.line, inline, this.trailingString);
  ...
  initialize (editor, inline) {
    ...
    this.indentSpaces = this.repeat(' ', Math.max(0, (config.get('indentation_spaces') || 1)));
