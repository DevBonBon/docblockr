 ...
 // have \u200B as a variable
  static config = {
    ...
    tags: {
      ...
      type: 'string',
      test: JSON.stringify({
        name: '${identifier/(.*)/[(?1:$1 )description]/}\n',
  ...
  static insert (cursor, snippet, docblock) {
    return this.insertSnippet
      ? this.insertSnippet(snippet, cursor, docblock)
      ...
      if (text.slice(0, column).endsWith(opener.slice(0, -1))) {
        ...
        if (!blacklist.includes(typeId || -1)) {
          ...
          return cursor.editor.buffer.transact(() => {
            ...
            const snippet = `${character}\${1: ${text.slice(column)}} ${closer}`;
            return this.insert(cursor, snippet);
          });
        }
      }
    }).length;
  ...
  static comment (whitelist, { comments, ...documenter }, inline) {
    ...
    return cursors.filter(cursor => {
      ...
      if (whitelist.includes(typeId || -1)) {
        let scope = this.commentScopes.uninitialized;
          ...
          for (const [opener, closer = ''] of comments[type]) {
            ...
            scope |= text.startsWith(opener) && text.endsWith(closer)
              ...
              ? /\S/.test(text.slice(opener.length, closer.length))
                ? this.commentScopes[type] & this.commentScopes.initialized
                : this.commentScopes[type]
              : 0;
          }
        }
        // Shift-enter should do nothing when inside an inline comment
        scope ^= inline ? this.commentScopes.inline : 0;

        const indent = cursor.editor.getTabText().repeat(cursor.editor.indentationForBufferRow(row))

        config.get('triggers').forEach((scope, snippet) => {

        })

        return scope
          ? this.insert(cursor, (new Docblock(scope, documenter))
