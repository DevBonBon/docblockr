...
class Docblockr {
  ...
  getIndentSpaces (editor, line) {
    const hasTypes = Docblockr.parsers[editor.getGrammar().scopeName].settings.typeInfo;
  ...
  initialize (editor, inline) {
    ...
    this.parser = Docblockr.parsers[editor.getGrammar().scopeName];

    this.indentSpaces = this.repeat(' ', Math.max(0, (config.get('indentation_spaces') || 1)));
    this.prefix = Docblockr.parsers[editor.getGrammar().scopeName].settings.prefix || ' *';
...
Docblockr.parsers = new Proxy(require('./grammars.json'), {
  get (target, scope) {
    ...
    return !target[grammar]
      ? (target[grammar] = new (require(`./grammars/${grammar}`))())
      : target[grammar];
  }
});
