...
class Docblockr {
  static get config () {
    return {
      ...
      docblock: {
        description: 'The overall format of generated docblocks. See package docs.',
        type: 'string',
      ...
      templates: {
        description: 'Templates for different block types seen in `format`. See package docs.',
        type: 'string',
        default: JSON.stringify({
          name: [
            ['', ' '],
          ...
          param: [
            ['\n * @', '\t{', '}\t', '', '', '\t\fdescription\n'],
            ['tag', '=', 'type', '\ftype', '&', 'value', '[', 'identifier', '&', 'value', [['=', ']'], ['value']]]
          ],
          returns: [
            ['\n * @', '\t{', '}\tdescription\n'],
            ['tag', '=', 'type', '\ftype']
  ...
  static document (inline = false, cursors = Docblockr.matchingCursors(/^\s*\/\*\*/, /\*\//, 'comment.block')) {
    return cursors.length && cursors.every(cursor => {
      ...
      const [template, placeholders] = JSON.parse(config.get('docblock'));
      const templates = JSON.parse(config.get('templates'));

      console.log(template, placeholders.map(placeholder =>
