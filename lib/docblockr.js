...
module.exports = class Docblockr {
  ...
  /**
   * A field and accessor for the package config schema
   * @type {Object}
   */
  static #CONFIG = Object.freeze({
    ...
    extend_triple_slash: {
      title: 'Extend triple-slash comments',
      description: 'Whether pressing enter on a \'///\' line should also comment out the next.',
      type: 'boolean',
      default: true,
      order: 18
  ...
  /**
   * A field and consumer for the Snippets service
   * @type {Object}
   */
  ...
  /**
   * Getter for the 'insertSnippet' function from the Snippets API Object
   * A getter is used so a notification can be issued if the service is unavailable
   * @type {Object}
   */
  static get insert () {
    if (Docblockr.#SNIPPETS == null) {
    ...
    return Docblockr.#SNIPPETS?.insertSnippet;
  }
  ...
  static get parser () {
    ...
    atom.config.unset('docblockr.simple_mode', { scopeSelector: `.${scopeName}` });
  ...
  static write (event, string) {
    ...
    this.insert?.(string, event.currentTarget.getModel()) ??
    event.abortKeyBinding();
  }
