...
module.exports = class DocsParser {
  ...
  static parse (line, nameOverride, inline) {
    if (config.get('simple_mode') || !line) {
      return null;
    }
    ...
    if (out) {
      return this.formatClass(...out);
    }
    out = this.parseFunction(line); // (name, args, retval, options)
    if (out) {
      return this.formatFunction(...out, nameOverride);
    }
    out = this.parseVar(line); // (name, val, valType)
    if (out) {
      return this.formatVar(inline, ...out);
    }
    return null;
  }

  static formatVar (inline, name, val, valType) {
  ...
  static geTypeInfo (argType, argName) {
    if (!this.settings.typeInfo) {
      return '';
    }
  ...
  static formatFunction (name, args, retval, options, nameOverride) {
    ...
    const extraTagAfter = config.get('extra_tags_go_after');

    const description = nameOverride || ('[' + escape(name) + (name ? ' ' : '') + 'description]');
    out.push('${1:' + description + '}');

    if (config.get('auto_add_method_tag')) {
    ...
    if (args) {
      ...
      for (i = 0; i < parsedArgs.length; i++) {
        ...
        if (config.get('param_description')) {
    ...
    if (retType !== null) {
      ...
      formatStr = `${config.get('return_tag')}${typeInfo}`;
      if (config.get('return_description')) {
        // the extra space here is so that the description will align with the param description
        let thirdArg = '';
        if (args && config.get('align_tags') === 'deep' && !config.get('per_section_indent')) {
          thirdArg = ' ';
        }
        formatStr += ` ${thirdArg}\${1:[description]}`;
      }
  ...
  static addExtraTags (out) {
    const extraTags = config.get('extra_tags');
    if (extraTags.length > 0) { out = out.concat(extraTags); }
    return out;
  }

  static guessTypeFromName (name) {
  ...
  static getMatchingNotations (name) {
    ...
    return config.get('notation_map').filter(checkMatch);
  }
  ...
  static getDefinition (editor, point) {
    ...
    while (maxLines-- > 0 && editor.getBuffer().getLastRow() >= point.row) {
      ...
      if (definition === '' && this.settings.fnOpener) {
        const match = RegExp(this.settings.fnOpener).exec(line);
