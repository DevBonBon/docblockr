...
class DocsParser {
  ...
  parse (line, inline, nameOverride) {
    ...
    if (out) {
      return this.formatFunction(nameOverride, ...out);
    }
  ...
  formatVar (inline, name, val, valType) {
  ...
  formatFunction (nameOverride, name, args, retval, options) {
    ...
    const extraTagAfter = config.get('extra_tags_go_after') || false;
