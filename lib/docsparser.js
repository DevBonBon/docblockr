...
class DocsParser {
  ...
  parse (line, inline) {
    ...
    if (out) {
      return this.formatVar(...out, inline);
    }
    return null;
  }

  formatVar (name, val, valType, inline) {
  ...
  geTypeInfo (argType, argName) {
    ...
    return `${braceOpen}\${1:${argType}}${braceClose} `;
  }
  ...
  formatFunction (name, args, retval, options) {
    ...
    const description = this.getNameOverride() || `[${name}${name ? ' ' : ''}description]`;
    ...
    if (args) {
      ...
      for (i = 0; i < parsedArgs.length; i++) {
        ...
        out.push(`@param ${typeInfo || ''}${argName} ${paramDescription || ''}`);
      }
    }
  ...
  addExtraTags (out) {
    return out.concat(config.get('extra_tags'));
  }
  ...
  getDefinition (cursor) {
    ...
    for (let i = 1; i < maxLines; i++) {
      line = cursor.editor.lineTextForBufferRow(cursor.getBufferRow() + i);
