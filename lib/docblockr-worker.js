...
module.exports = class DocBlockrAtom {
  ...
  static parseCommand (event, inline = false) {
    ...
    const parser = this.getParser(editor);
    ...
    const line = parser.getDefinition(editor, cursorPosition);
    ...
    // align the tags
    if (out && (shallowAlignTags || deepAlignTags) && !inline) {
      ...
      if (config.get('per_section_indent')) { returnTag = config.get('return_tag'); } else { returnTag = false; }
      ...
      const minColSpaces = config.get('min_spaces_between_columns');
      for (let i = 0; i < out.length; i++) {
        ...
        if ((out[i].startsWith('@')) && (!out[i].startsWith('@author'))) {
          ...
          for (let j = 0; j < splitArray.length; j++) {
            ...
            newOut.push(' '.repeat(minColSpaces) +
    ...
    if (inline) {
      ...
      if (out) {
        ...
        for (let i = 0; i < out.length; i++) {
          snippet += '\n' + prefix + (out[i] ? (indentSpaces + out[i]) : '');
        }
      } else {
  ...
  static getIndentSpaces (editor, line) {
  ...
  static getParser (editor) {
    ...
    if ((sourceLang === null) && (scope === 'text.html.php')) {
      return new Parsers.PhpParser();
    }

    if (sourceLang === 'coffee') {
      return new Parsers.CoffeeParser();
    } else if ((sourceLang === 'actionscript') || (sourceLang === 'haxe')) {
      return new Parsers.ActionscriptParser();
    } else if ((sourceLang === 'c++') || (sourceLang === 'cpp') || (sourceLang === 'c') || (sourceLang === 'cuda-c++')) {
      return new Parsers.CppParser();
    } else if ((sourceLang === 'objc') || (sourceLang === 'objc++')) {
      return new Parsers.ObjCParser();
    } else if ((sourceLang === 'java') || (sourceLang === 'groovy')) {
      return new Parsers.JavaParser();
    } else if (sourceLang === 'rust') {
      return new Parsers.RustParser();
    } else if (sourceLang === 'ts') {
      return new Parsers.TypescriptParser();
    } else if (sourceLang === 'processing') {
      return new Parsers.ProcessingParser();
    } else if (sourceLang === 'css' && subSourceLang === 'scss') {
      return new Parsers.SassParser();
    }
    return new Parsers.JsParser();
  }

  static substituteVariables (out) {
  ...
  static fixTabStops (out) {
    let counter = 0;
    const swapTabs = (match, group1, group2, str) => `${group1}${counter++}${group2}`;
    return out.map(line => line.replace(/(\$\{)\d+(:[^}]+\})/g, swapTabs));
  }
}
