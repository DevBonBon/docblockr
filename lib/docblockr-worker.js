...
class DocBlockrAtom {
  ...
  parseCommand (event, inline) {
    ...
    editor.getBuffer().delete(this.trailingRange);
  ...
  initialize (editor, inline) {
    ...
    this.indentSpaces = ' '.repeat(config.get('indentation_spaces'));
    ...
    this.line = this.parser.getDefinition(editor, cursorPosition);
  }
  ...
  generateSnippet (out, inline) {
    ...
    } else { return (this.createSnippet(out) + (config.get('newline_after_block') ? '\n' : '')); }
  }
